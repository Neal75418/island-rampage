...
use bevy::prelude::*;
use super::{UiText, TimeDisplay, HealthBarBg, MoneyDisplay, MissionInfo, MinimapContainer, MinimapPlayerMarker, PauseMenu, ResumeButton, QuitButton, FullMapContainer, FullMapPlayerMarker, UiState, ChineseFont, EnemyHealthBar, EnemyHealthBarFill, EnemyHealthBarHighlight, EnemyHealthBarGlow, DamageIndicator, DamageIndicatorEdge, DamageEdge, DamageIndicatorState};
use super::{PlayerStatusContainer, HealthBarFill, HealthBarHighlight, HealthLabel, ArmorSection, ArmorBarFill, ArmorLabel, WeaponSlot, CurrentAmmoText, ReserveAmmoText, WeaponAreaContainer, ControlHintContainer, ControlStatusTag, ControlSpeedDisplay, ControlKeyArea};
use super::{HudAnimationState, CrosshairDynamics, HealthBarGlow, MinimapScanLine, MinimapPlayerGlow, WeaponSwitchAnimation};
use super::{HealthLabelShadow, ArmorLabelShadow, WeaponDisplayShadow, CurrentAmmoShadow, ReserveAmmoShadow};
use super::{AmmoVisualGrid, AmmoBulletIcon};
use super::{WeatherHudContainer, WeatherIconContainer, WeatherIconElement, WeatherIconType, SunRay, RainDropIcon, WeatherNameText};
...
pub fn setup_ui(mut commands: Commands, chinese_font: Res<ChineseFont>) {
        ...
        parent.spawn((
            ...
            row.spawn((
                ...
                label_container.spawn((
                    Text::new("100/100"),
                    TextFont { font_size: 14.0, font: font.clone(), ..default() },
        ...
        glow.spawn((
            ...
            frame.spawn((
                ...
                parent.spawn((
                    ...
                    title_bg.spawn((
                        Text::new("西門町"),
                        TextFont { font_size: 10.0, font: font.clone(), ..default() },
                ...
                spawn_map_layer(parent, mm_scale, mm_off_x, mm_off_y, mw_fac, false, font.clone());
                ...
                parent.spawn((
                    ...
                    bg.spawn((
                        Text::new("N"),
                        TextFont { font_size: 13.0, font: font.clone(), ..default() },
    ...
    commands.spawn((
        ...
        parent.spawn((
            Text::new("$ 5,000"),
            TextFont { font_size: 28.0, font: font.clone(), ..default() },
    ...
    commands.spawn((
        ...
        parent.spawn((
            ...
            tag.spawn((
                Text::new("步行"),
                TextFont { font_size: 12.0, font: font.clone(), ..default() },
        ...
        parent.spawn((
            ...
            // 按鍵圖示 helper
            let spawn_key = |keys: &mut ChildSpawnerCommands, key: &str, action: &str, font: Handle<Font>| {
                // 按鍵背景
                keys.spawn((
                    Node {
                    ...
                    key_bg.spawn((
                        Text::new(key),
                        TextFont { font_size: 11.0, font: font.clone(), ..default() },
        ...
        parent.spawn((
            ...
            glow.spawn((
                ...
                border.spawn((
                    Node {
                        flex_direction: FlexDirection::Column,
                        padding: UiRect::new(Val::Px(50.0), Val::Px(50.0), Val::Px(35.0), Val::Px(35.0)),
                        row_gap: Val::Px(18.0),
                        align_items: AlignItems::Center,
                        border: UiRect::all(Val::Px(1.0)),
                        ..default()
                    ...
                    menu.spawn((
                        ...
                        title_area.spawn((
                            Text::new("遊戲暫停"),
                            TextFont { font_size: 32.0, font: font.clone(), ..default() },
                    ...
                    menu.spawn((
                        Node {
                            width: Val::Px(220.0),
                            height: Val::Px(1.0),
                            margin: UiRect::new(Val::Px(0.0), Val::Px(0.0), Val::Px(5.0), Val::Px(10.0)),
                            ..default()
                        },
    ...
    commands.spawn((
        ...
        parent.spawn((
            ...
            title_row.spawn((
                Node {
                    padding: UiRect::new(Val::Px(30.0), Val::Px(30.0), Val::Px(10.0), Val::Px(10.0)),
                    border: UiRect::all(Val::Px(2.0)),
                    ..default()
                },
        ...
        parent.spawn((
            ...
            glow.spawn((
                ...
                frame.spawn((
                    ...
                    spawn_map_layer(map, fm_scale, fm_off_x, fm_off_y, fw_fac, true, font.clone());
                    ...
                    map.spawn((
                        ...
                        bg.spawn((
                            Text::new("N"),
                            TextFont { font_size: 22.0, font: font.clone(), ..default() },
                            TextColor(COMPASS_NORTH),
                        ));
                    });
                    // 南
...
/// 生成圖例項目
fn spawn_legend_item(parent: &mut ChildSpawnerCommands, color: Color, label: &str, font: Handle<Font>) {
    parent.spawn((
        Node {
            flex_direction: FlexDirection::Row,
            align_items: AlignItems::Center,
        ...
        item.spawn((
            Text::new(label),
            TextFont { font_size: 12.0, font, ..default() },
            TextColor(Color::WHITE),
        ));
    });
}
...
pub fn update_ui(
    ...
    mut text_query: Query<&mut Text, (With<UiText>, Without<TimeDisplay>, Without<ControlStatusTag>, Without<ControlSpeedDisplay>)>,
    mut time_query: Query<&mut Text, With<TimeDisplay>>,
    mut status_tag_query: Query<&mut Text, (With<ControlStatusTag>, Without<TimeDisplay>, Without<UiText>, Without<ControlSpeedDisplay>)>,
    mut speed_display_query: Query<(&mut Text, &mut Visibility), (With<ControlSpeedDisplay>, Without<TimeDisplay>, Without<UiText>, Without<ControlStatusTag>)>,
) {
    // 更新舊版控制提示文字（保留兼容）
...
/// 取得控制提示文字
fn get_control_hint_text(game_state: &GameState, vehicle_query: &Query<&Vehicle>, mission_manager: &MissionManager) -> String {
    if !game_state.player_in_vehicle {
        ...
        if let Some(ref active) = mission_manager.active_mission {
            if let Ok(player_transform) = player_query.single() {
                ...
                if let Some(limit) = active.data.time_limit {
                    ...
                    **text = format!("[任務] {} | {:.0}m | {:.0}s", active.data.title, distance, remaining);
                } else {
...
pub fn update_hud(
    ...
    mut health_shadow_query: Query<&mut Text, (With<HealthLabelShadow>, Without<HealthLabel>, Without<ArmorLabel>, Without<ArmorLabelShadow>)>,
    mut armor_section_query: Query<&mut Visibility, With<ArmorSection>>,
    mut armor_fill_query: Query<&mut Node, (With<ArmorBarFill>, Without<HealthBarFill>, Without<HealthBarHighlight>)>,
    mut armor_label_query: Query<&mut Text, (With<ArmorLabel>, Without<HealthLabel>, Without<MoneyDisplay>, Without<ArmorLabelShadow>)>,
    mut armor_shadow_query: Query<&mut Text, (With<ArmorLabelShadow>, Without<ArmorLabel>, Without<HealthLabel>, Without<HealthLabelShadow>)>,
    mut money_query: Query<&mut Text, (With<MoneyDisplay>, Without<HealthLabel>, Without<ArmorLabel>, Without<HealthLabelShadow>, Without<ArmorLabelShadow>)>,
) {
    let Ok((health, armor_opt, player)) = player_query.single() else { return; };
...
pub fn toggle_pause(
    ...
    let quit_pressed = is_button_pressed(&quit_query)
        || (ui_state.paused && keyboard.just_pressed(KeyCode::KeyQ));
    if quit_pressed {
        std::process::exit(0);
    }
    ...
    if keyboard.just_pressed(KeyCode::Escape) {
        set_pause_state(!ui_state.paused, &mut ui_state, &mut pause_menu_query, &mut time);
    }
}
...
pub fn button_hover_effect(
    mut resume_query: Query<(&Interaction, &mut BackgroundColor), (Changed<Interaction>, With<ResumeButton>)>,
    mut quit_query: Query<(&Interaction, &mut BackgroundColor), (Changed<Interaction>, With<QuitButton>, Without<ResumeButton>)>,
) {
    // 繼續遊戲按鈕
...
pub fn update_minimap(
    ...
    mut player_marker_query: Query<(&mut Node, &mut Transform), (With<MinimapPlayerMarker>, Without<Player>)>,
) {
    // 獲取玩家在 3D 世界的位置和方向
...
pub fn update_fullmap(
    ...
    mut fullmap_marker_query: Query<(&mut Node, &mut Transform), (With<FullMapPlayerMarker>, Without<Player>)>,
) {
    // 獲取玩家在 3D 世界的位置和方向
...
pub fn setup_world_name_tags(
    ...
    missions: Query<(Entity, &GlobalTransform, &crate::mission::MissionMarker), Added<crate::mission::MissionMarker>>,
) {
    let font = asset_server.load("fonts/STHeiti.ttc");
    
    // 建築物標籤 (白色)
    for (entity, _transform, building) in &buildings {
        if building.name.is_empty() { continue; }
...
fn spawn_map_layer(
    ...
    // 引用世界常數 (更新為新的道路佈局)
    use crate::world::{
        X_ZHONGHUA, X_HAN, X_XINING, X_KANGDING,
        Z_HANKOU, Z_WUCHANG, Z_KUNMING, Z_EMEI, Z_CHENGDU,
        W_ZHONGHUA, W_MAIN, W_SECONDARY, W_PEDESTRIAN, W_ALLEY
    };
    ...
    // 南北向道路 (Vertical)
    draw_road_rect(parent, X_ZHONGHUA, -15.0, W_ZHONGHUA * road_width_factor, v_len_main, scale, off_x, off_y, is_fullmap, "中華路", font.clone());
    draw_road_rect(parent, X_XINING, -15.0, W_SECONDARY * road_width_factor, v_len_main, scale, off_x, off_y, is_fullmap, "西寧南路", font.clone());
    draw_road_rect(parent, X_KANGDING, -15.0, W_MAIN * road_width_factor, v_len_main, scale, off_x, off_y, is_fullmap, "康定路", font.clone());
    draw_road_rect(parent, X_HAN, 0.0, W_PEDESTRIAN * road_width_factor, 100.0, scale, off_x, off_y, is_fullmap, "漢中街", font.clone());

    // 東西向道路 (Horizontal)
    let h_len = 200.0;
    draw_road_rect(parent, h_center_x, Z_HANKOU, h_len, W_SECONDARY * road_width_factor, scale, off_x, off_y, is_fullmap, "漢口街", font.clone());
    draw_road_rect(parent, h_center_x, Z_WUCHANG, h_len, W_PEDESTRIAN * road_width_factor, scale, off_x, off_y, is_fullmap, "武昌街", font.clone());
    draw_road_rect(parent, h_center_x, Z_KUNMING, h_len, W_ALLEY * road_width_factor, scale, off_x, off_y, is_fullmap, "昆明街", font.clone());
    draw_road_rect(parent, h_center_x, Z_EMEI, h_len, W_PEDESTRIAN * road_width_factor, scale, off_x, off_y, is_fullmap, "峨嵋街", font.clone());
    draw_road_rect(parent, h_center_x, Z_CHENGDU, h_len, W_MAIN * road_width_factor, scale, off_x, off_y, is_fullmap, "成都路", font.clone());

    // 2. 繪製地標 (Landmarks) - 根據新的建築位置更新
    let landmarks = [
        // 西寧南路沿線
        MapLandmark { name: "萬年",    world_x: X_XINING - 16.0, world_z: Z_EMEI - 17.5, w: 20.0, d: 15.0, color: Color::srgb(0.5, 0.5, 0.7) },
        MapLandmark { name: "獅子林",  world_x: X_XINING - 17.0, world_z: Z_WUCHANG - 18.5, w: 22.0, d: 22.0, color: Color::srgb(0.5, 0.4, 0.3) },
        MapLandmark { name: "Donki",  world_x: X_XINING + 20.0, world_z: Z_WUCHANG + 18.5, w: 28.0, d: 22.0, color: Color::srgb(1.0, 0.85, 0.0) },
        MapLandmark { name: "電影公園", world_x: X_XINING - 18.5, world_z: Z_KUNMING - 14.0, w: 25.0, d: 20.0, color: Color::srgb(0.25, 0.4, 0.25) },
        // 漢中街沿線
        MapLandmark { name: "誠品西門", world_x: X_HAN - 16.5, world_z: Z_EMEI - 15.5, w: 18.0, d: 16.0, color: ESLITE_GREEN },
        MapLandmark { name: "誠品武昌", world_x: X_HAN - 14.5, world_z: Z_WUCHANG + 14.5, w: 14.0, d: 14.0, color: ESLITE_GREEN },
        MapLandmark { name: "UQ",     world_x: X_HAN + 13.5, world_z: Z_EMEI - 15.0, w: 12.0, d: 12.0, color: Color::srgb(0.85, 0.15, 0.15) },
        MapLandmark { name: "H&M",    world_x: X_HAN + 14.5, world_z: Z_CHENGDU - 15.0, w: 14.0, d: 14.0, color: Color::srgb(0.85, 0.85, 0.85) },
        // 中華路沿線
        MapLandmark { name: "捷運6號", world_x: X_ZHONGHUA - 26.0, world_z: Z_CHENGDU - 14.0, w: 12.0, d: 12.0, color: Color::srgb(0.2, 0.35, 0.65) },
        MapLandmark { name: "紅樓",   world_x: X_ZHONGHUA - 31.0, world_z: Z_CHENGDU + 19.0, w: 22.0, d: 22.0, color: Color::srgb(0.7, 0.22, 0.18) },
        MapLandmark { name: "錢櫃",   world_x: X_ZHONGHUA + 28.0, world_z: Z_CHENGDU - 16.0, w: 16.0, d: 16.0, color: Color::srgb(0.75, 0.45, 0.55) },
        MapLandmark { name: "鴨肉扁", world_x: X_ZHONGHUA - 25.0, world_z: Z_WUCHANG + 12.5, w: 10.0, d: 10.0, color: Color::srgb(0.85, 0.65, 0.35) },
        // 康定路沿線
        MapLandmark { name: "西門國小", world_x: X_KANGDING + 23.0, world_z: Z_WUCHANG - 20.0, w: 30.0, d: 25.0, color: Color::srgb(0.7, 0.65, 0.55) },
    ];
    ...
    for lm in landmarks.iter() {
        if is_fullmap {
            ...
            draw_building_rect(parent, lm.world_x, lm.world_z, lm.w, lm.d, scale, off_x, off_y, lm.color, lm.name, font.clone());
        } else {
            // 小地圖：顯示簡化點
            draw_minimap_point(parent, lm.world_x, lm.world_z, scale, off_x, off_y, lm.color, lm.name, font.clone());
        }
    }
}
...
fn draw_road_rect(
    parent: &mut ChildSpawnerCommands, 
    x: f32, z: f32, width: f32, length: f32, // w=thickness, l=length
    scale: f32, off_x: f32, off_y: f32, 
    show_name: bool, name: &str, font: Handle<Font>
) {
    ...
    spawn_centered_rect(parent, ui_x, ui_y, ui_w, ui_h, Color::srgba(0.5, 0.5, 0.55, 0.6), if show_name { name } else { "" }, 14.0, font);
}

#[allow(clippy::too_many_arguments)]
fn draw_building_rect(
    parent: &mut ChildSpawnerCommands,
    x: f32, z: f32, w: f32, d: f32, // World dims
    scale: f32, off_x: f32, off_y: f32,
    color: Color, name: &str, font: Handle<Font>
) {
    let ui_w = w * scale;
    let ui_h = d * scale;
    let ui_x = x * scale + off_x;
...
fn spawn_centered_rect(
    parent: &mut ChildSpawnerCommands,
    ui_x: f32, ui_y: f32, ui_w: f32, ui_h: f32,
    color: Color, name: &str, font_size: f32, font: Handle<Font>
) {
    parent.spawn((
        Node {
            position_type: PositionType::Absolute,
...
fn draw_minimap_point(
    ...
    parent.spawn((
        Text::new(name),
        TextFont { font_size: 8.0, font, ..default() },
        Node {
            position_type: PositionType::Absolute,
...
fn update_rating_display(
    rating_query: &mut Query<&mut Text, (With<super::DeliveryRatingDisplay>, Without<super::DeliveryStreakDisplay>)>,
    mission_manager: &MissionManager,
) {
    let Ok(mut text) = rating_query.single_mut() else { return; };
...
fn update_streak_display(
    streak_query: &mut Query<&mut Text, (With<super::DeliveryStreakDisplay>, Without<super::DeliveryRatingDisplay>)>,
    streak: u32,
) {
    let Ok(mut text) = streak_query.single_mut() else { return; };
    **text = if streak > 0 {
        format!("x{} 連擊", streak)
    } else {
        "x0 連擊".to_string()
...
pub fn update_delivery_app(
    ...
    mut rating_query: Query<&mut Text, (With<super::DeliveryRatingDisplay>, Without<super::DeliveryStreakDisplay>)>,
    mut streak_query: Query<&mut Text, (With<super::DeliveryStreakDisplay>, Without<super::DeliveryRatingDisplay>)>,
) {
    if !ui_state.show_delivery_app {
        return;
    }
    ...
    // 生成新卡片
    let Ok(list_entity) = order_list_query.single() else { return; };
...
use super::{Crosshair, CrosshairDot, CrosshairLine, CrosshairDirection, CrosshairOuterRing, CrosshairHitMarker, HitMarkerLine, WeaponDisplay};
    ...
    commands.spawn((
        ...
        parent.spawn((
            ...
            crosshair.spawn((
                ...
                CrosshairLine { direction: CrosshairDirection::Top },
            ));
            ...
            crosshair.spawn((
                ...
                BackgroundColor(CROSSHAIR_MAIN),
                BorderRadius::all(Val::Px(1.0)),
                CrosshairLine { direction: CrosshairDirection::Bottom },
            ));
            ...
            crosshair.spawn((
                ...
                BackgroundColor(CROSSHAIR_MAIN),
                BorderRadius::all(Val::Px(1.0)),
                CrosshairLine { direction: CrosshairDirection::Left },
            ));
            ...
            crosshair.spawn((
                ...
                BackgroundColor(CROSSHAIR_MAIN),
                BorderRadius::all(Val::Px(1.0)),
                CrosshairLine { direction: CrosshairDirection::Right },
            ));
    ...
    commands.spawn((
        ...
        parent.spawn((
            ...
            ammo_row.spawn((
                ...
                label_container.spawn((
                    Text::new("12"),
                    TextFont { font_size: 36.0, font: font.clone(), ..default() },
                    TextColor(TEXT_SHADOW_COLOR),
                    Node {
                        position_type: PositionType::Absolute,
        ...
        parent.spawn((
            ...
            for i in 0..4 {
                ...
                slots.spawn((
                    ...
                    BackgroundColor(if is_active { SLOT_ACTIVE } else { SLOT_INACTIVE }),
...
pub fn update_crosshair(
    ...
    // 瞄準時中心點變亮
    let dot_color = if is_aiming { CROSSHAIR_AIM } else { CROSSHAIR_MAIN };
...
pub fn update_ammo_display(
    player_query: Query<&WeaponInventory, With<Player>>,
    mut current_ammo_query: Query<(&mut Text, &mut TextColor), (With<CurrentAmmoText>, Without<ReserveAmmoText>, Without<WeaponDisplay>, Without<CurrentAmmoShadow>, Without<ReserveAmmoShadow>, Without<WeaponDisplayShadow>)>,
    mut current_ammo_shadow_query: Query<&mut Text, (With<CurrentAmmoShadow>, Without<CurrentAmmoText>, Without<ReserveAmmoText>, Without<ReserveAmmoShadow>, Without<WeaponDisplay>, Without<WeaponDisplayShadow>)>,
    mut reserve_ammo_query: Query<&mut Text, (With<ReserveAmmoText>, Without<CurrentAmmoText>, Without<WeaponDisplay>, Without<ReserveAmmoShadow>, Without<CurrentAmmoShadow>, Without<WeaponDisplayShadow>)>,
    mut reserve_ammo_shadow_query: Query<&mut Text, (With<ReserveAmmoShadow>, Without<ReserveAmmoText>, Without<CurrentAmmoText>, Without<CurrentAmmoShadow>, Without<WeaponDisplay>, Without<WeaponDisplayShadow>)>,
    mut weapon_query: Query<&mut Text, (With<WeaponDisplay>, Without<CurrentAmmoText>, Without<ReserveAmmoText>, Without<WeaponDisplayShadow>, Without<CurrentAmmoShadow>, Without<ReserveAmmoShadow>)>,
...
pub fn update_ammo_visual_grid(
    ...
    let Ok(children) = children_query.get(grid_entity) else { return; };
    ...
    if existing_count != magazine_size && magazine_size > 0 {
        ...
        commands.entity(grid_entity).with_children(|grid| {
            for i in 0..magazine_size {
                ...
                let color = if is_filled { BULLET_FILLED } else { BULLET_EMPTY };
                grid.spawn((
                    Node {
                        width: Val::Px(4.0),
                        height: Val::Px(10.0),
...
pub fn update_weapon_switch_animation(
    ...
    mut weapon_display_query: Query<&mut TextColor, (With<WeaponDisplay>, Without<CurrentAmmoText>, Without<ReserveAmmoText>, Without<WeaponDisplayShadow>, Without<CurrentAmmoShadow>, Without<ReserveAmmoShadow>)>,
    mut weapon_shadow_query: Query<&mut TextColor, (With<WeaponDisplayShadow>, Without<WeaponDisplay>, Without<CurrentAmmoText>, Without<ReserveAmmoText>, Without<CurrentAmmoShadow>, Without<ReserveAmmoShadow>)>,
    mut current_ammo_query: Query<&mut TextColor, (With<CurrentAmmoText>, Without<WeaponDisplay>, Without<ReserveAmmoText>, Without<CurrentAmmoShadow>, Without<ReserveAmmoShadow>, Without<WeaponDisplayShadow>)>,
    mut current_shadow_query: Query<&mut TextColor, (With<CurrentAmmoShadow>, Without<CurrentAmmoText>, Without<WeaponDisplay>, Without<ReserveAmmoText>, Without<ReserveAmmoShadow>, Without<WeaponDisplayShadow>)>,
    mut reserve_ammo_query: Query<&mut TextColor, (With<ReserveAmmoText>, Without<CurrentAmmoText>, Without<WeaponDisplay>, Without<ReserveAmmoShadow>, Without<CurrentAmmoShadow>, Without<WeaponDisplayShadow>)>,
    mut reserve_shadow_query: Query<&mut TextColor, (With<ReserveAmmoShadow>, Without<ReserveAmmoText>, Without<CurrentAmmoText>, Without<CurrentAmmoShadow>, Without<WeaponDisplay>, Without<WeaponDisplayShadow>)>,
...
pub fn update_enemy_health_bars(
    ...
    mut bar_query: Query<(&mut Node, &mut Visibility, &EnemyHealthBar), Without<EnemyHealthBarFill>>,
    mut fill_query: Query<(&mut Node, &mut BackgroundColor, &EnemyHealthBarFill), Without<EnemyHealthBar>>,
) {
    let Ok((camera, camera_transform)) = camera_query.single() else { return; };
...
pub fn setup_damage_indicator(mut commands: Commands) {
    ...
    commands.spawn((
        ...
        parent.spawn((
            ...
            DamageIndicatorEdge { edge: DamageEdge::Top },
        ));
        ...
        parent.spawn((
            ...
            BackgroundColor(DAMAGE_INDICATOR_COLOR),
            DamageIndicatorEdge { edge: DamageEdge::Bottom },
        ));
        ...
        parent.spawn((
            ...
            BackgroundColor(DAMAGE_INDICATOR_COLOR),
            DamageIndicatorEdge { edge: DamageEdge::Left },
        ));
        ...
        parent.spawn((
            ...
            BackgroundColor(DAMAGE_INDICATOR_COLOR),
            DamageIndicatorEdge { edge: DamageEdge::Right },
        ));
...
pub fn update_hud_animations(
    ...
    mut player_glow_query: Query<&mut BackgroundColor, (With<MinimapPlayerGlow>, Without<HealthBarGlow>)>,
) {
...
pub fn update_crosshair_dynamics(
    ...
    if dynamics.hit_bounce_scale > 1.0 {
        dynamics.hit_bounce_scale -= (dynamics.hit_bounce_scale - 1.0) * CROSSHAIR_HIT_BOUNCE_RECOVERY * dt;
...
fn spawn_sun_icon(parent: &mut ChildSpawnerCommands) {
    parent.spawn((
        ...
        WeatherIconElement { weather_type: WeatherIconType::Sun },
    )).with_children(|sun| {
        // 外發光
        sun.spawn((
            Node {
...
fn spawn_cloud_icon(parent: &mut ChildSpawnerCommands) {
    parent.spawn((
        ...
        Visibility::Hidden,
        WeatherIconElement { weather_type: WeatherIconType::Cloud },
    )).with_children(|cloud| {
        // 雲朵由多個圓組成
        // 左側小圓
        cloud.spawn((
...
fn spawn_rain_icon(parent: &mut ChildSpawnerCommands) {
    parent.spawn((
        ...
        Visibility::Hidden,
        WeatherIconElement { weather_type: WeatherIconType::Rain },
    )).with_children(|rain| {
        // 深色雲
        rain.spawn((
            Node {
        ...
        for i in 0..3 {
            ...
            rain.spawn((
                ...
                BorderRadius::all(Val::Px(1.5)),
                RainDropIcon { index: i, offset: i as f32 * 0.3 },
            ));
        }
    });
}
...
fn spawn_fog_icon(parent: &mut ChildSpawnerCommands) {
    parent.spawn((
        ...
        Visibility::Hidden,
        WeatherIconElement { weather_type: WeatherIconType::Fog },
    )).with_children(|fog| {
        // 三條橫線代表霧
        for i in 0..3 {
            let width = match i {
...
use super::{InteractionPromptContainer, InteractionPromptKey, InteractionPromptText, InteractionPromptState};
use crate::mission::MissionTrigger;
...
/// 設置互動提示 UI
pub fn setup_interaction_prompt(
    mut commands: Commands,
    chinese_font: Res<ChineseFont>,
) {
    // 外層容器（全寬，用於居中）
...
pub fn update_interaction_prompt_state(
    ...
    for (trigger_transform, trigger) in &trigger_query {
        ...
        // 檢查玩家是否在觸發範圍內
        let in_range = trigger.shape.contains(trigger_transform.translation, player_pos);
        if !in_range {
            continue;
        }
    ...
    if let Some((trigger, _)) = closest_trigger {
        let prompt_text = trigger.prompt_text.clone().unwrap_or_else(|| "按 F 互動".to_string());
...
use super::{GpsNavigationState, MinimapGpsMarker, GpsDirectionArrow, GpsDistanceDisplay};
...
pub fn update_gps_navigation(
    ...
    player_query: Query<&Transform, With<Player>>,
    mut arrow_query: Query<(&mut Visibility, &mut Transform, &Children), (With<GpsDirectionArrow>, Without<Player>)>,
    mut distance_query: Query<(&mut Visibility, &Children), (With<GpsDistanceDisplay>, Without<GpsDirectionArrow>, Without<Player>)>,
...
pub fn update_minimap_gps_marker(
    ...
    existing_markers: Query<Entity, With<MinimapGpsMarker>>,
) {
    // 移除舊標記
    ...
    // 找到小地圖容器
    let Ok(minimap_entity) = minimap_query.single() else { return };
...
/// 設置劇情任務 HUD
pub fn setup_story_mission_hud(
    mut commands: Commands,
    font: Option<Res<ChineseFont>>,
) {
    let Some(font) = font else { return };
...
pub fn update_story_mission_hud(
    ...
    mut title_query: Query<&mut Text, (With<StoryMissionTitle>, Without<StoryMissionPhaseText>, Without<StoryMissionTimer>, Without<StoryMissionObjectiveCheck>, Without<StoryMissionObjectiveText>)>,
    mut phase_query: Query<&mut Text, (With<StoryMissionPhaseText>, Without<StoryMissionTitle>, Without<StoryMissionTimer>, Without<StoryMissionObjectiveCheck>, Without<StoryMissionObjectiveText>)>,
    mut timer_query: Query<&mut Text, (With<StoryMissionTimer>, Without<StoryMissionTitle>, Without<StoryMissionPhaseText>, Without<StoryMissionObjectiveCheck>, Without<StoryMissionObjectiveText>)>,
