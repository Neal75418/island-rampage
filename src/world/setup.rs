...
use crate::world::{Building, BuildingType, BuildingWindow, StreetFurniture, StreetFurnitureType, StreetLight, WorldMaterials, PlayerInteriorState, InteriorSpace, Door};
use crate::player::{Player, DodgeState};
use crate::vehicle::{Vehicle, spawn_scooter};
use crate::combat::{WeaponInventory, Weapon, WeaponStats, Health, Armor, Damageable, PlayerArm, PlayerHand, HitReaction};
use crate::core::{COLLISION_GROUP_CHARACTER, COLLISION_GROUP_VEHICLE, COLLISION_GROUP_STATIC};
use crate::world::{NeonSign, spawn_neon_sign};
...
pub fn setup_world(
    ...
    // 中華路 (東邊界) - 主幹道，貫穿南北
    spawn_road_segment(&mut commands, &mut meshes, &mut materials, road_mat.clone(), line_mat.clone(),
    ...
    spawn_road_segment(&mut commands, &mut meshes, &mut materials, pedestrian_mat.clone(), line_mat.clone(),
    ...
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        ...
        Z_EMEI, W_PEDESTRIAN, -1.0,
        20.0, 28.0, 15.0, "萬年大樓", Color::srgb(0.45, 0.45, 0.5));
    ...
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        X_XINING, W_SECONDARY, 1.0,
        Z_WUCHANG, W_PEDESTRIAN, 1.0,
        28.0, 35.0, 22.0, "Don Don Donki", Color::srgb(1.0, 0.85, 0.0));

    // ========== 漢中街沿線建築 ==========

    // 誠品西門店 (峨嵋北側, 漢中西側)
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        X_HAN, W_PEDESTRIAN, -1.0,
        Z_EMEI, W_PEDESTRIAN, -1.0,
        18.0, 20.0, 16.0, "誠品西門", Color::srgb(0.2, 0.35, 0.25));

    // 誠品武昌店 (武昌南側, 漢中西側)
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        X_HAN, W_PEDESTRIAN, -1.0,
        Z_WUCHANG, W_PEDESTRIAN, 1.0,
        14.0, 18.0, 14.0, "誠品武昌", Color::srgb(0.2, 0.35, 0.25));

    // Uniqlo (漢中東側，峨嵋北側)
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        X_HAN, W_PEDESTRIAN, 1.0,
        Z_EMEI, W_PEDESTRIAN, -1.0,
        12.0, 15.0, 12.0, "Uniqlo", Color::srgb(0.85, 0.15, 0.15));

    // H&M (漢中東側，成都北側)
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        X_HAN, W_PEDESTRIAN, 1.0,
        Z_CHENGDU, W_MAIN, -1.0,
        14.0, 18.0, 14.0, "H&M", Color::srgb(0.85, 0.85, 0.85));

    // ========== 中華路沿線建築 ==========

    // 捷運西門站6號出口 (中華/成都 NW 角)
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        X_ZHONGHUA, W_ZHONGHUA, -1.0,
        Z_CHENGDU, W_MAIN, -1.0,
        12.0, 8.0, 12.0, "捷運6號出口", Color::srgb(0.2, 0.35, 0.65));

    // 西門紅樓 (中華路西側，成都路南側) - 歷史地標
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        X_ZHONGHUA, W_ZHONGHUA, -1.0,
        Z_CHENGDU, W_MAIN, 1.0,
        22.0, 14.0, 22.0, "西門紅樓", Color::srgb(0.7, 0.22, 0.18));

    // 錢櫃 KTV (中華路東側，成都路北側)
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        X_ZHONGHUA, W_ZHONGHUA, 1.0,
        Z_CHENGDU, W_MAIN, -1.0,
        16.0, 22.0, 16.0, "錢櫃KTV", Color::srgb(0.75, 0.45, 0.55));

    // 鴨肉扁 (中華路西側，武昌街南側) - 著名小吃
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        X_ZHONGHUA, W_ZHONGHUA, -1.0,
        Z_WUCHANG, W_PEDESTRIAN, 1.0,
        10.0, 8.0, 10.0, "鴨肉扁", Color::srgb(0.85, 0.65, 0.35));

    // 新光三越 (中華路西側，峨嵋北側) - 大型百貨
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        X_ZHONGHUA, W_ZHONGHUA, -1.0,
        Z_EMEI, W_PEDESTRIAN, -1.0,
        18.0, 28.0, 16.0, "新光三越", Color::srgb(0.85, 0.85, 0.9));
    ...
    spawn_rich_building(&mut commands, &mut meshes, &mut materials,
        ...
        16.0, 30.0, 14.0, "統一元氣館");

    // 遠東百貨 (中華路東側，漢口街南側) - 大型商場
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        X_ZHONGHUA, W_ZHONGHUA, 1.0,
        Z_HANKOU, W_SECONDARY, 1.0,
        20.0, 25.0, 18.0, "遠東百貨", Color::srgb(0.3, 0.4, 0.6));

    // 台北車站方向商業大樓 (中華路東側，武昌北側)
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        X_ZHONGHUA, W_ZHONGHUA, 1.0,
        Z_WUCHANG, W_PEDESTRIAN, -1.0,
        14.0, 20.0, 12.0, "商業大樓A", Color::srgb(0.6, 0.6, 0.65));

    // ========== 成都路沿線建築 ==========

    // 阿宗麵線 (成都路北側，西寧與漢中之間)
    spawn_building_at_linear(&mut commands, &mut meshes, &mut materials,
        Z_CHENGDU, W_MAIN, -1.0,  // 成都路北側
        X_XINING, X_HAN,          // 西寧到漢中之間
        8.0, 6.0, "阿宗麵線", Color::srgb(0.9, 0.5, 0.2));
    ...
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        X_KANGDING, W_MAIN, 1.0,
        Z_EMEI, W_PEDESTRIAN, -1.0,
        12.0, 10.0, 12.0, "7-ELEVEN", Color::srgb(0.2, 0.5, 0.35));

    // === 4. 裝飾：便利商店與小店 (沿街填充) ===
    // 漢中街西側 - KFC
    spawn_building_at_linear(&mut commands, &mut meshes, &mut materials,
        X_HAN, W_PEDESTRIAN, -1.0, Z_EMEI, Z_CHENGDU, 6.0, 6.0, "KFC", Color::srgb(0.8, 0.1, 0.1));

    // 峨嵋街南側 - 小吃攤
    spawn_building_at_linear(&mut commands, &mut meshes, &mut materials,
        Z_EMEI, W_PEDESTRIAN, 1.0, X_XINING, X_HAN, 5.0, 5.0, "小吃街", Color::srgb(0.85, 0.65, 0.4));
    ...
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        X_XINING, W_SECONDARY, 1.0,
        Z_HANKOU, W_SECONDARY, 1.0,
        10.0, 8.0, 10.0, "全家便利", Color::srgb(0.2, 0.5, 0.4));

    // 麥當勞 (漢口街南側，漢中西側)
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        X_HAN, W_PEDESTRIAN, -1.0,
        Z_HANKOU, W_SECONDARY, 1.0,
        14.0, 12.0, 12.0, "麥當勞", Color::srgb(0.95, 0.75, 0.1));
    ...
    spawn_building_at_corner(&mut commands, &mut meshes, &mut materials,
        X_KANGDING, W_MAIN, 1.0,
        Z_CHENGDU, W_MAIN, -1.0,
        14.0, 12.0, 14.0, "彈珠台", Color::srgb(0.8, 0.7, 0.2));
    ...
    spawn_player_character(
        ...
        &mut materials,
        start_pos,
        Player { speed: 8.0, rotation_speed: 3.0, money: 5000, ..default() },
    );
    ...
    for (pos, rot, variant) in vending_positions {
        spawn_vending_machine(&mut commands, &mut meshes, &mut materials, pos, rot, variant);
    }
    ...
    for (cx, cz, road_ns_w, road_ew_w, _name) in intersections {
        ...
        // 北側斑馬線 (東西向)
        spawn_zebra_crossing(&mut commands, &mut meshes, &zebra_mat,
    ...
    let billboard_configs = [
        (Vec3::new(25.0, 8.0, -58.0), Color::srgb(1.0, 0.3, 0.2), "動作片"),   // 紅色
        (Vec3::new(35.0, 8.0, -58.0), Color::srgb(0.2, 0.5, 1.0), "科幻片"),   // 藍色
        (Vec3::new(45.0, 8.0, -58.0), Color::srgb(1.0, 0.8, 0.2), "喜劇片"),   // 金色
        (Vec3::new(55.0, 8.0, -58.0), Color::srgb(0.6, 0.1, 0.8), "恐怖片"),   // 紫色
    ];
...
impl RoadLayout {
    fn new(width_x: f32, width_z: f32) -> Self {
        ...
        Self { is_horizontal, road_len, total_width }
    }
}
...
fn spawn_building_at_corner(
    ...
    meshes: &mut ResMut<Assets<Mesh>>,
    materials: &mut ResMut<Assets<StandardMaterial>>,
    road1_center: f32, road1_width: f32, align1: f32, // align: -1 (Low coord), 1 (High coord)
    road2_center: f32, road2_width: f32, align2: f32,
...
fn spawn_building_at_linear(
    ...
    meshes: &mut ResMut<Assets<Mesh>>,
    materials: &mut ResMut<Assets<StandardMaterial>>,
    road_center: f32, road_width: f32, align: f32, // 主路 alignment
    start_cross: f32, end_cross: f32, // 橫路範圍
    width: f32, depth: f32,
    name: &str, _color: Color  // 保留參數但標記為未使用
) {
    // 放在兩條橫路的中間
...
fn spawn_vehicle(commands: &mut Commands, meshes: &mut ResMut<Assets<Mesh>>, materials: &mut ResMut<Assets<StandardMaterial>>, pos: Vec3, v_type: Vehicle, color: Color) {
    use crate::vehicle::{VehicleType, VehicleHealth};
    ...
    commands.spawn((
        ...
        RigidBody::Dynamic, // 閒置車輛用 Dynamic
        LockedAxes::ROTATION_LOCKED_X | LockedAxes::ROTATION_LOCKED_Z, // 防止翻車
        CollisionGroups::new(
...
fn match_building_type(name: &str) -> BuildingStyle {
    ...
    const PATTERNS: &[(&[&str], BuildingStyle)] = &[
        ...
        (&["全家", "7-ELEVEN", "50嵐"], BuildingStyle::ConvenienceStore),
...
fn spawn_building_base<'a>(
    ...
    cmd.spawn((
        ...
        Building { name: params.name.to_string(), building_type: BuildingType::Shop },
    ))
}
...
fn spawn_rich_building(
    ...
    match match_building_type(name) {
        BuildingStyle::Wannien => spawn_wannien(commands, meshes, materials, pos, width, height, depth, name),
        BuildingStyle::Donki => spawn_donki(commands, meshes, materials, pos, width, height, depth, name),
        BuildingStyle::Eslite => spawn_eslite(commands, meshes, materials, pos, width, height, depth, name),
        BuildingStyle::ModernGrid => spawn_modern_grid(commands, meshes, materials, pos, width, height, depth, name),
        BuildingStyle::Cinema => spawn_cinema(commands, meshes, materials, pos, width, height, depth, name),
        BuildingStyle::Hotel => spawn_hotel(commands, meshes, materials, pos, width, height, depth, name),
        BuildingStyle::GameCenter => spawn_game_center(commands, meshes, materials, pos, width, height, depth, name),
        BuildingStyle::TattooShop => spawn_tattoo_shop(commands, meshes, materials, pos, width, height, depth, name),
        BuildingStyle::FastFood => spawn_fast_food(commands, meshes, materials, pos, width, height, depth, name),
        BuildingStyle::ConvenienceStore => spawn_convenience_store(commands, meshes, materials, pos, width, height, depth, name),
        BuildingStyle::StreetWear => spawn_streetwear_shop(commands, meshes, materials, pos, width, height, depth, name),
        BuildingStyle::ClawMachine => spawn_claw_machine(commands, meshes, materials, pos, width, height, depth, name),
        BuildingStyle::Daiso => spawn_daiso(commands, meshes, materials, pos, width, height, depth, name),
        BuildingStyle::Generic => spawn_generic_building(commands, meshes, materials, pos, width, height, depth, name),
    }
}

// === 1. 萬年大樓 (轉角圓柱風格) ===
fn spawn_wannien(cmd: &mut Commands, meshes: &mut ResMut<Assets<Mesh>>, mats: &mut ResMut<Assets<StandardMaterial>>, pos: Vec3, w: f32, h: f32, d: f32, name: &str) {
    ...
    cmd.spawn((
        ...
        Building { name: name.to_string(), building_type: BuildingType::Shop },
    )).with_children(|parent| {
        ...
        parent.spawn((
            ...
            Transform::from_xyz(w/2.0 - corner_radius, h/2.0 + 2.0, d/2.0 - corner_radius),
            GlobalTransform::default(),
        ));
...
// === 2. 唐吉訶德 (雜亂招牌風格) ===
fn spawn_donki(cmd: &mut Commands, meshes: &mut ResMut<Assets<Mesh>>, mats: &mut ResMut<Assets<StandardMaterial>>, pos: Vec3, w: f32, h: f32, d: f32, name: &str) {
    ...
    spawn_building_base(cmd, meshes, mats, &params, config).with_children(|parent| {
        // 生成大量隨機突出的招牌
        let sign_mat_1 = mats.add(StandardMaterial { base_color: Color::srgb(0.0, 0.0, 0.0), emissive: LinearRgba::new(0.1, 0.1, 0.1, 1.0), ..default() });
        let sign_mat_2 = mats.add(StandardMaterial { base_color: Color::srgb(0.0, 0.0, 1.0), emissive: LinearRgba::new(0.0, 0.0, 1.0, 1.0) * 3.0, ..default() });
        ...
        for i in 0..10 {
            ...
            parent.spawn((
                ...
                MeshMaterial3d(if is_blue { sign_mat_2.clone() } else { sign_mat_1.clone() }),
        ...
        parent.spawn((
            ...
            MeshMaterial3d(mats.add(StandardMaterial { base_color: Color::srgb(0.0, 0.0, 1.0), emissive: LinearRgba::new(0.0, 0.0, 1.0, 1.0) * 2.0, ..default() })),
...
// === 3. 誠品 (植生牆風格) ===
fn spawn_eslite(cmd: &mut Commands, meshes: &mut ResMut<Assets<Mesh>>, mats: &mut ResMut<Assets<StandardMaterial>>, pos: Vec3, w: f32, h: f32, d: f32, name: &str) {
    let params = BuildingParams { pos, w, h, d, name };
    let config = BuildingMaterialConfig {
    ...
    spawn_building_base(cmd, meshes, mats, &params, config).with_children(|parent| {
        // 木紋/植生凸起
        let wood_mat = mats.add(StandardMaterial { base_color: Color::srgb(0.4, 0.25, 0.1), ..default() });
        for i in 0..5 {
...
// === 4. 現代網格 (H&M / Uniqlo) ===
fn spawn_modern_grid(cmd: &mut Commands, meshes: &mut ResMut<Assets<Mesh>>, mats: &mut ResMut<Assets<StandardMaterial>>, pos: Vec3, w: f32, h: f32, d: f32, name: &str) {
    let main_color = if name.contains("H&M") { Color::srgb(1.0, 1.0, 1.0) } else { Color::srgb(0.9, 0.9, 0.9) };
    ...
    cmd.spawn((
        ...
        // 外部格柵 (白色)
        let white_mat = mats.add(StandardMaterial { base_color: main_color, ..default() });
        ...
        parent.spawn((
            ...
            MeshMaterial3d(mats.add(StandardMaterial { base_color: accent_color, emissive: LinearRgba::from(accent_color) * 4.0, ..default() })),
...
// === 5. 電影院 (Cinema) ===
fn spawn_cinema(cmd: &mut Commands, meshes: &mut ResMut<Assets<Mesh>>, mats: &mut ResMut<Assets<StandardMaterial>>, pos: Vec3, w: f32, h: f32, d: f32, name: &str) {
    let params = BuildingParams { pos, w, h, d, name };
    let config = BuildingMaterialConfig {
...
// === 6. 飯店 (Hotel) ===
fn spawn_hotel(cmd: &mut Commands, meshes: &mut ResMut<Assets<Mesh>>, mats: &mut ResMut<Assets<StandardMaterial>>, pos: Vec3, w: f32, h: f32, d: f32, name: &str) {
    let params = BuildingParams { pos, w, h, d, name };
    let config = BuildingMaterialConfig {
    ...
    spawn_building_base(cmd, meshes, mats, &params, config).with_children(|parent| {
        // 陽台 (Balconies)
        let balcony_mat = mats.add(StandardMaterial { base_color: Color::srgb(0.1, 0.1, 0.1), ..default() });
        ...
        // 頂樓招牌 (HOTEL)
        let sign_mat = mats.add(StandardMaterial { base_color: Color::srgb(1.0, 0.5, 0.0), emissive: LinearRgba::new(1.0, 0.5, 0.0, 1.0) * 5.0, ..default() });
        parent.spawn((
...
// === 7. 通用建築 (形狀變體) ===
fn spawn_generic_building(cmd: &mut Commands, meshes: &mut ResMut<Assets<Mesh>>, mats: &mut ResMut<Assets<StandardMaterial>>, pos: Vec3, w: f32, h: f32, d: f32, name: &str) {
    use rand::Rng;
    let mut rng = rand::rng();
    let shape_type = rng.random_range(0..3); // 0: Box, 1: Stepped, 2: Twin

    let color = Color::srgb(rng.random_range(0.2..0.5), rng.random_range(0.2..0.5), rng.random_range(0.2..0.5));
    let main_mat = mats.add(StandardMaterial { base_color: color, perceptual_roughness: 0.8, ..default() });
...
fn spawn_game_center(cmd: &mut Commands, meshes: &mut ResMut<Assets<Mesh>>, mats: &mut ResMut<Assets<StandardMaterial>>, pos: Vec3, w: f32, h: f32, d: f32, name: &str) {
    ...
    let params = BuildingParams { pos, w, h, d, name };
    let config = BuildingMaterialConfig { base_color: main_color, ..default() };
    spawn_building_base(cmd, meshes, mats, &params, config).with_children(|parent| {
        // 閃爍的霓虹燈條
...
// === 9. 刺青店 (Tattoo Shop) ===
fn spawn_tattoo_shop(cmd: &mut Commands, meshes: &mut ResMut<Assets<Mesh>>, mats: &mut ResMut<Assets<StandardMaterial>>, pos: Vec3, w: f32, h: f32, d: f32, name: &str) {
    let params = BuildingParams { pos, w, h, d, name };
    let config = BuildingMaterialConfig {
...
fn spawn_fast_food(cmd: &mut Commands, meshes: &mut ResMut<Assets<Mesh>>, mats: &mut ResMut<Assets<StandardMaterial>>, pos: Vec3, w: f32, h: f32, d: f32, name: &str) {
...
fn spawn_convenience_store(cmd: &mut Commands, meshes: &mut ResMut<Assets<Mesh>>, mats: &mut ResMut<Assets<StandardMaterial>>, pos: Vec3, w: f32, h: f32, d: f32, name: &str) {
...
fn spawn_streetwear_shop(cmd: &mut Commands, meshes: &mut ResMut<Assets<Mesh>>, mats: &mut ResMut<Assets<StandardMaterial>>, pos: Vec3, w: f32, h: f32, d: f32, name: &str) {
    ...
    spawn_building_base(cmd, meshes, mats, &params, config).with_children(|parent| {
        ...
        // 紅色 Logo 標誌
        let accent = if name.contains("潮牌") { Color::srgb(1.0, 0.1, 0.1) }
                     else if name.contains("球鞋") { Color::srgb(1.0, 0.5, 0.0) }
                     else { Color::srgb(0.8, 0.6, 0.3) };
        parent.spawn((
...
fn spawn_claw_machine(cmd: &mut Commands, meshes: &mut ResMut<Assets<Mesh>>, mats: &mut ResMut<Assets<StandardMaterial>>, pos: Vec3, w: f32, h: f32, d: f32, name: &str) {
    let params = BuildingParams { pos, w, h, d, name };
    let config = BuildingMaterialConfig {
...
// === 14. 大創 (Daiso) ===
fn spawn_daiso(cmd: &mut Commands, meshes: &mut ResMut<Assets<Mesh>>, mats: &mut ResMut<Assets<StandardMaterial>>, pos: Vec3, w: f32, h: f32, d: f32, name: &str) {
    let params = BuildingParams { pos, w, h, d, name };
    let config = BuildingMaterialConfig {
...
fn spawn_graffiti_wall(
    ...
    commands.spawn((
        ...
        let splash_positions: [(f32, f32, usize); 15] = [
            (-5.0, 1.0, 0), (2.0, 1.5, 1), (5.5, 0.5, 2),
            (-3.0, -1.0, 3), (0.0, 0.0, 4), (4.0, -0.5, 5),
            (-6.0, -1.5, 6), (6.0, 1.0, 7), (-1.0, 2.0, 0),
            (3.0, -1.5, 1), (-4.5, 0.5, 2), (1.5, -2.0, 3),
            (-2.0, 1.8, 4), (5.0, -1.8, 5), (-5.5, -0.5, 6),
        ];
...
fn spawn_parking_garage(cmd: &mut Commands, meshes: &mut ResMut<Assets<Mesh>>, mats: &mut ResMut<Assets<StandardMaterial>>, pos: Vec3, w: f32, h: f32, d: f32, name: &str) {
    // 主體結構 (開放式)
    cmd.spawn((
        Mesh3d(meshes.add(Cuboid::new(w, h, d))),
        MeshMaterial3d(mats.add(StandardMaterial { base_color: Color::srgb(0.5, 0.5, 0.55), ..default() })),
        Transform::from_translation(pos),
        GlobalTransform::default(),
        Collider::cuboid(w/2.0, h/2.0, d/2.0),
        Building { name: name.to_string(), building_type: BuildingType::Shop },
        ...
        let floor_mat = mats.add(StandardMaterial { base_color: Color::srgb(0.3, 0.3, 0.3), ..default() });
...
pub fn spawn_player_character(
    ...
    let player_entity = commands.spawn((
        ...
        KinematicCharacterController { slide: true, ..default() },
        player,
    ...
    .with_children(|parent| {
        ...
        parent.spawn((
            ...
            MeshMaterial3d(skin_mat.clone()),
            Transform::from_xyz(ear_x, HEAD_Y, 0.0)
        ...
        parent.spawn((
            ...
            MeshMaterial3d(hair_mat.clone()),
            Transform::from_xyz(0.0, HEAD_Y + 0.02, -0.08)
                .with_scale(Vec3::new(0.9, 0.8, 0.6)),
        ));
        // 瀏海
        parent.spawn((
